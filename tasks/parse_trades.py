...
def parse_trades_ten_seconds():
    ...
    from tasks.transform_trade_data import PRICE, QUANTITY, DEFAULT_COL_SEARCH
    ...
    if not (ts_begin := optional_add_secs_in_ms((get_last_ts_from_db(connect_to_n_ms_parsed_trades_db(secs_parse_interval), DEFAULT_COL_SEARCH)), ms_parse_interval)):
        ts_begin = round_to_last_n_secs(get_last_ts_from_db(connect_to_aggtrade_data_db(), DEFAULT_COL_SEARCH), secs_parse_interval)
