...
def transform_trade_data(args):
    ...
    if args['multithread_start_end_timeframe']:
        ...
        finish_ts = args['multithread_start_end_timeframe'][1]
    else:
        finish_ts = 0
        if not (start_ts := ValidatorDB(chart_tf_db).end_ts) and not (start_ts := ValidatorDB(TEN_SECS_PARSED_TRADES_DB).start_ts):
    ...
    def calc_finish_ts(finish_ts):
        if not finish_ts:
            finish_ts = ValidatorDB(TEN_SECS_PARSED_TRADES_DB).end_ts
        elif ValidatorDB(TEN_SECS_PARSED_TRADES_DB).end_ts < finish_ts + mins_to_ms(args['chart_minutes']):
            finish_ts = ValidatorDB(TEN_SECS_PARSED_TRADES_DB).end_ts - mins_to_ms(args['chart_minutes'])
        else:
            finish_ts += mins_to_ms(args['chart_minutes'])
        return finish_ts

    LOG.info("Starting to transform trade data for interval '%s' minutes, a start timestamp of '%s' and finish timestamp of '%s'.",
             args['chart_minutes'], datetime.fromtimestamp(start_ts / 1000), datetime.fromtimestamp(calc_finish_ts(finish_ts) / 1000))
    ...
    if args['multithread_start_end_timeframe']:
        LOG.info(f"Parsed trade data from {args['multithread_start_end_timeframe'][0]} to "
                 f"{args['multithread_start_end_timeframe'][1]} for a {args['chart_minutes']} timeframe.")
        exit(0)
    else:
