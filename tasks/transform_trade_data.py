...
def create_refresh_cache_future_trades(parsing_ts, symbols_to_parse) -> dict:
    ...
    trade_data_group = TradeDataGroup(TRADE_DATA_CACHE_TIME_IN_MINUTES,
                           parsing_ts + mins_to_ms(
                               TRADE_DATA_CACHE_TIME_IN_MINUTES) + DEFAULT_PARSE_INTERVAL_IN_MS,
...
def transform_trade_data(args):
    ...
    begin_ts, finish_ts = round_last_ten_secs(args['start_end_timeframe'][0]), round_last_ten_secs(
        args['start_end_timeframe'][1])
    ...
    parsing_ts = begin_ts
    maximum_atomicity_timeframe = max([trade_chart_atomicity.value.atomicity for trade_chart_atomicity in
                                       TradesChartTimeframeValuesAtomicity])
    while parsing_ts <= finish_ts:
        if parsing_ts > list(cache_future_trades[DEFAULT_COL_SEARCH])[-1] - (maximum_atomicity_timeframe + DEFAULT_PARSE_INTERVAL_IN_MS):
            cache_future_trades = create_refresh_cache_future_trades(parsing_ts - DEFAULT_PARSE_INTERVAL_IN_MS, TRANSFORM_TRADE_DATA_USED_SYMBOLS)

        for timeframe in TRADES_CHART_TIMEFRAMES_VALUES:
            if not parsing_ts % TRADES_CHART_TF_ATOMICITY[timeframe]:
                try:
