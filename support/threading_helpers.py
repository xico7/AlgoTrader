    ...
    for start_end_tf in timeframes:
        threads.append(threading.Thread(
            target=run_algotrader_process, args=(
                ...
                algotrader_process_args + ['--start-end-timeframe', f'{start_end_tf[0]}', f'{start_end_tf[1]}']
            )
        ))
...
def create_run_timeframe_chart_threads(thread_number: int = 3):
    ...
    if ValidatorDB(TEN_SECS_PARSED_TRADES_DB).finish_ts < begin_ts + parse_time * thread_number:
    ...
    return create_run_threads(thread_number, begin_ts, parse_time, 'transform-trade-data')


def create_run_metrics_parser_threads(threads_number, parse_time, metric_db_mapper_name, begin_ts):
    return create_run_threads(
        threads_number, begin_ts, parse_time, 'metrics-parser', ['--metric-db-mapper-name', metric_db_mapper_name])